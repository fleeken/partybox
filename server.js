...
function publicRoom(room) {
  return {
    code: room.code,
    phase: room.phase,
    players: room.players.map(p => ({name:p.name,color:p.color,score:p.score,voted:p.voted,ready:p.ready,stopped:Boolean(room.game?.stops?.has(p.id))})),
    voteOptions: room.voteOptions,
    selectedGame: room.selectedGame,
    game: room.game ? {name:room.game.name,status:room.game.status,startsAt:room.game.startsAt,revealUntil:room.game.revealUntil,stoppedCount:room.game.stops.size,results:room.game.results||[]} : null
  };
}
function emitRoom(room) { io.to(room.code).emit('room:update', publicRoom(room)); }

function beginPractice(room, gameName) {
  room.selectedGame = gameName;
  room.phase = 'practice';
  room.players.forEach(p => p.ready = false);
  room.game = { name: gameName, status:'practice', startsAt:Date.now(), revealUntil:0, stops:new Map(), results:[] };
  emitRoom(room);
}
...
function finishTen(room) {
  ...
  room.game.status='results'; room.game.results=results; room.phase='results'; emitRoom(room);
}

function startRealTen(room) {
  const now = Date.now();
  room.phase='game';
  room.players.forEach(p=>p.ready=false);
  room.game={name:'Stoppa på 10.00',status:'countdown',startsAt:now+3500,revealUntil:now+5500,stops:new Map(),results:[]};
  emitRoom(room);
  setTimeout(()=>{if(rooms.has(room.code)&&room.game?.name==='Stoppa på 10.00'){room.game.status='running';emitRoom(room);}},3500);
...
io.on('connection', socket => {
  ...
  socket.on('player:join', ({roomCode,name}, reply=()=>{}) => {
    ...
    const player={id:socket.id,name:cleanName,color:colors[room.players.length],score:0,voted:false,ready:false};
  ...
  socket.on('host:startVote', (_, reply=()=>{}) => {
    const room=rooms.get(socket.data.roomCode); if(!room||room.hostId!==socket.id||!room.players.length)return reply({ok:false});
    room.phase='voting'; room.game=null; room.selectedGame=null; room.votes.clear(); room.players.forEach(p=>{p.voted=false;p.ready=false});
    room.voteOptions=[...games].sort(()=>Math.random()-.5).slice(0,3); emitRoom(room); reply({ok:true});
  });
  ...
  socket.on('player:vote', ({game}, reply=()=>{}) => {
    ...
    if(room.players.length&&room.votes.size===room.players.length){
      ...
      const max=Math.max(...Object.values(counts)); const tied=room.voteOptions.filter(g=>counts[g]===max); const winner=tied[randomInt(tied.length)];
      room.selectedGame=winner; room.phase='selected'; io.to(room.code).emit('vote:result',{winner,counts}); emitRoom(room);
    }
  });

  socket.on('host:startSelected', (_, reply=()=>{}) => {
    const room=rooms.get(socket.data.roomCode); if(!room||room.hostId!==socket.id)return reply({ok:false});
    if(room.selectedGame!=='Stoppa på 10.00')return reply({ok:false,error:'Det spelet är inte byggt ännu.'});
    beginPractice(room,room.selectedGame); reply({ok:true});
  });

  socket.on('host:startTen', (_, reply=()=>{}) => {
    const room=rooms.get(socket.data.roomCode); if(!room||room.hostId!==socket.id||!room.players.length)return reply({ok:false});
    beginPractice(room,'Stoppa på 10.00'); reply({ok:true});
  });

  socket.on('player:ready', (_, reply=()=>{}) => {
    const room=rooms.get(socket.data.roomCode); const player=room&&findPlayer(room,socket.id);
    if(!room||!player||room.phase!=='practice')return reply({ok:false});
    player.ready=true; emitRoom(room); reply({ok:true});
    if(room.players.length&&room.players.every(p=>p.ready)){
      if(room.selectedGame==='Stoppa på 10.00') startRealTen(room);
    }
  });

  socket.on('player:stopTen', (_, reply=()=>{}) => {
    const room=rooms.get(socket.data.roomCode); const player=room&&findPlayer(room,socket.id);
    if(!room||!player||room.phase!=='game'||room.game?.name!=='Stoppa på 10.00')return reply({ok:false});
    if(room.game.stops.has(socket.id))return reply({ok:false});
    const elapsed=Date.now()-room.game.startsAt; if(elapsed<0)return reply({ok:false,error:'För tidigt!'});
    room.game.stops.set(socket.id,Math.min(elapsed,15000)); reply({ok:true}); emitRoom(room);
    if(room.game.stops.size===room.players.length)finishTen(room);
  });

  socket.on('disconnect',()=>{
    const room=rooms.get(socket.data.roomCode); if(!room)return;
    if(room.hostId===socket.id){if(room.game?.finishTimer)clearTimeout(room.game.finishTimer);io.to(room.code).emit('room:closed');rooms.delete(room.code);return;}
    room.players=room.players.filter(p=>p.id!==socket.id); room.votes.delete(socket.id); room.game?.stops?.delete(socket.id);
    if(room.phase==='practice'&&room.players.length&&room.players.every(p=>p.ready)&&room.selectedGame==='Stoppa på 10.00')startRealTen(room);
    else if(room.phase==='game'&&room.players.length&&room.game?.stops.size===room.players.length)finishTen(room); else emitRoom(room);
  });
});
